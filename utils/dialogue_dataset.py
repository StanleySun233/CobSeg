...
class Utterance:
    def __init__(self, dial_id, utterances, segments, utt_lst, set_split="train"):
...
class DialogueDataset(Dataset):
    ...
    def _normalize_item(self, raw: dict) -> dict:
        ...
        # 1) extract dial_id
        dial_id = ''
        for key in ('dial_id', 'dialogue_id', 'id', 'conversation_id', 'conv_id'):
            value = raw.get(key)
        ...
        set_split = str(raw.get('set', 'train')).strip()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='vfh', help='vfh | dialseg711 | doc2dial or path to json')
